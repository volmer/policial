...
describe Policial::StyleGuides::Ruby, '#violations_in_file' do
...
private
  ...
  def two
    ...
    context 'for trailing dot used for multi-line method chain' do
      it 'returns violations' do
        expect(violations_in(<<-CODE)).not_to be_empty
one.
  two.
...
def register_email(email:)
  ...
  context 'with custom configuration' do
    ...
    context 'with old-style syntax' do
      it 'has one violation' do
        ...
        expect(violations).to eq [
          'Prefer double-quoted strings unless you need single '\
  ...
  def violations_in(content, config = nil)
    ...
    style_guide = Policial::StyleGuides::Ruby.new(repo_config)
